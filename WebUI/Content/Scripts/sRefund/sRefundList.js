...
firstFunction = function () {
    ...
    Easy.bindSaveButtonClickEvent("#editsRefund", "#fSave", "../sRefund/GetsRefundEdit", "#btnSave", "1", "#grid", menuId, "1", "#sRefundID");
    Easy.bindSaveUploadFile("#uploadsRefund", "#fUpload", "../sRefund/UpLoadsRefund", "#btnSaveUploadsRefund", "1", "#grid", menuId, "1", "upload");
};
initGrid = function (queryData) {
    $("#grid").datagrid({
        ...
        url: "../sRefund/GetsRefundList", sortName: "sRefundID", sortOrder: "desc",
...
upload = function () {
    if (Easy.bindPowerValidationEvent(menuId, "1", "upload")) {
        Easy.OpenDialogEvent("#uploadsRefund", "导入核销信息", 400, 300, "../sRefund/sRefundUp", "#uploadsRefund-buttons");
    }
};
add = function () {
    if (Easy.bindPowerValidationEvent(menuId, "1", "add")) {
        sRefundId = "0";
        Easy.OpenDialogEvent("#editsRefund", "编辑核销信息", 680, 300, "../sRefund/sRefundEdit", "#editsRefund-buttons");
    }
};
edit = function () {
    Easy.centerShow("系统消息","核销信息不可修改",2000);
};
enable = function () {
...
bindUpdateStatus = function (confirmstr, status) {
    ...
    if (Easy.checkRow(rows, "核销信息")) {
        //修改状态
        Easy.bindUpdateValue(confirmstr, status, "../sRefund/UpdateStatus", rows[0].sRefundID, "1", "#grid");
    }
}
